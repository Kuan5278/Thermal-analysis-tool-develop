# thermal_analysis_platform_v10.3.10_final.py
# 溫度數據視覺化平台 - v10.3.10 (修復YOKOGAWA時間範圍錯誤)
...
# 版本資訊
VERSION = "v10.3.10 Final (Fixed Time Range Error)"
...
class LogData:
    ...
    def get_time_range(self) -> Tuple[float, float]:
        ...
        try:
            # 計算最大秒數，並確保它是一個有效的有限數值
            max_seconds = self.df.index.total_seconds().max()
            
            # 如果 max_seconds 是 NaN 或 無窮大，返回一個安全的預設範圍
            if not np.isfinite(max_seconds):
                return (0.0, 0.0)
                
            return (0.0, max_seconds)
        except Exception:
            # 如果在計算過程中發生任何其他錯誤，也返回安全的預設值
            return (0.0, 0.0)
...
class YokogawaParser(LogParser):
    ...
    def parse(self, file_content: io.BytesIO, filename: str) -> Optional[LogData]:
        try:
            ...
            try: 
                df['time_index'] = pd.to_timedelta(df[time_column].astype(str).str.strip() + ':00')
            except: 
                time_col_dt = pd.to_datetime(df[time_column], errors='coerce')
                df['time_index'] = time_col_dt - time_col_dt.dropna().iloc[0]
...
class SystemLogRenderer:
    ...
    def render(self, file_index=None):
        ...
        for i in range(max_rows):
            ...
            if i < len(cpu_cores):
                core = cpu_cores[i]
                if core in freq_stats and pd.notna(freq_stats[core]['max']):
            ...
            if i < len(cpu_cores) + len(other_components):
                comp = cpu_cores[i] if i < len(cpu_cores) else other_components[i - len(cpu_cores)]
                if comp in temp_stats and pd.notna(temp_stats[comp]['max']):
