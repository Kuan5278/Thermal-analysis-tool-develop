# universal_analysis_platform_v7_2_final.py
...
def parse_dispatcher(uploaded_file):
    ...
    try:
        read_func = pd.read_excel if is_excel else pd.read_csv
        df_sniff = read_func(file_content, header=None, skiprows=27, nrows=1)
        file_content.seek(0)
        for cell in df_sniff.iloc[0]:
...
def generate_yokogawa_temp_chart(df, x_limits=None):
    ...
    cols_to_plot = [col for col in numeric_cols if all(s not in col for s in ['Date', 'sec', 'RT', 'TIME'])]
    
    for col in cols_to_plot:
        ax.plot(df_chart.index.total_seconds(), pd.to_numeric(df_chart[col], errors='coerce'), label=col, linewidth=1.5)
...
if uploaded_files:
    ...
    if is_single_yokogawa:
        ...
        # 關鍵修正：為YOKOGAWA專屬模式加上時間拉桿
        st.sidebar.header("X軸範圍設定 (秒)")
        x_min_val = df_yoko.index.min().total_seconds(); x_max_val = df_yoko.index.max().total_seconds()
        ...
        fig = generate_yokogawa_temp_chart(df_yoko, x_limits=(x_min, x_max))
