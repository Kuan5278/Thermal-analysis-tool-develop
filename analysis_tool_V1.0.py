...
class PTATParser(LogParser):
    ...
    def parse(self, file_content: io.BytesIO, filename: str) -> Optional[LogData]:
        ...
        try:
            ...
            time_series_cleaned = time_series.str.replace(r':(\d{3})
...
class StatisticsCalculator:
    ...
    def calculate_ptat_stats(log_data: LogData, x_limits=None):
        """計算PTAT統計數據 - 頻率單位維持MHz"""
        df = log_data.filter_by_time(x_limits)
        if df.empty:
            return None, None, None
        
        # CPU Core Frequency 統計 - 維持MHz單位
        freq_stats = []
...
class PTATRenderer:
    ...
    def render_controls(self, file_index=None):
        ...
        if left_y_range_enabled:
            ...
            with col1:
                left_y_min = st.number_input("左Y軸最小值", value=0.0, step=1.0, key=f"{key_prefix}left_y_min")
            with col2:
                left_y_max = st.number_input("左Y軸最大值", value=100.0, step=1.0, key=f"{key_prefix}left_y_max")
        ...
        if right_y_axis and right_y_axis != 'None':
            ...
            if right_y_range_enabled:
                col1, col2 = st.sidebar.columns(2)
                with col1:
                    right_y_min = st.number_input("右Y軸最小值", value=0.0, step=1.0, key=f"{key_prefix}right_y_min")
                with col2:
                    right_y_max = st.number_input("右Y軸最大值", value=100.0, step=1.0, key=f"{key_prefix}right_y_max")
    ...
    def render(self, file_index=None):
        ...
        <div style="background-color: #d1ecf1; border: 1px solid #bee5eb; color: #0c5460; padding: 1rem; border-radius: 8px; margin: 1rem 0;">
            ...
            <p>已識別為CPU性能監控數據，包含頻率、功耗、溫度等指標 (頻率單位：MHz)</p>
        </div>
...
class RendererFactory:
    ...
    def create_renderer(log_data: LogData):
        ...
        if log_type == "Burn-In Test Log":
            ...
            return PTATRenderer(log_data)  # PTAT頻率單位維持MHz
...
def display_version_info():
    ...
    with st.expander("📋 版本資訊", expanded=False):
        ...
        **當前版本：{VERSION}** | **發布日期：{VERSION_DATE}**
        ...
        - **⚡ CPU頻率監控** - 每個核心的頻率變化曲線 (燒機測試單位：kHz，PTAT維持MHz)
        - **📊 統計分析** - 最大、最小、平均溫度/頻率
        - **🔄 多檔案比較** - 自動整合多次燒機測試結果
        - **⏱️ 時間範圍選擇** - 可自定義分析時間區間
        - **📏 Y軸自由調整** - 完全開放Y軸範圍設定
